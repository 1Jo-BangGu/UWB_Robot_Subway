...
if __name__ == "__main__":
    client = RaspiVisionClient("192.168.4.8")
    client.start()
    ...
    while True:
        ...
        #             # msg = f'{left}, {right}'
        #             # client.send_message('control', msg)

        if transform_calculated and obstacle_input_done and goal_input_done and M_pixel2real is not None:
            result = detect_robot_position(frame, M_pixel2real)
            if result is not None and result[0] is not None:
                robot_pose, (cx, cy) = result
                robot_position = robot_pose  # 새로 감지된 위치 저장
            elif robot_position is not None:
                robot_pose = robot_position
                cx, cy = 0, 0  # 이전 좌표 사용 시에는 표시 위치 없음
            else:
                robot_pose = None  # 위치 정보 없음

            if robot_pose is not None:
                x, y, yaw = robot_pose
                cv2.putText(frame, f"X:{x:.1f} Y:{y:.1f} Yaw:{yaw:.1f}", (cx+10, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,0), 2)
                if goal_list_cm:
                    goal = find_nearest_goal((x, y), goal_list_cm)
                    latest_path = a_star((x, y), goal, (0, 180, 0, 90), obstacle_list)
                    left, right = follow_path(robot_pose, latest_path)
                    msg = f'{left}, {right}'
